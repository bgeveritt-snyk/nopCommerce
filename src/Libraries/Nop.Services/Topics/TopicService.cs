...
namespace Nop.Services.Topics
{
    ...
    public partial class TopicService : ITopicService
    {
        ...
        public bool ApplyTopicConstraints(int storeId, int[] customerRoleIds, out IQueryable<Topic> topicsQuery)
        {
            var constraintsFlag = false;
            topicsQuery = _topicRepository.Table.Where(p => p.Published);

            if (!_catalogSettings.IgnoreStoreLimitations && _storeMappingService.IsEntityMappingExists<Product>(storeId))
            {
                topicsQuery = topicsQuery.Where(_storeMappingService.ApplyStoreMapping<Topic>(storeId));
                constraintsFlag = true;
            }

            if (!_catalogSettings.IgnoreAcl && _aclService.IsEntityAclMappingExist<Topic>(customerRoleIds))
            {
                topicsQuery = topicsQuery.Where(_aclService.ApplyAcl<Topic>(customerRoleIds));
                constraintsFlag = true;
            }
        ...
        public virtual Topic GetTopicBySystemName(string systemName, int storeId = 0, bool showHidden = false)
        {
            ...
            var topic = _staticCacheManager.Get(cacheKey, () =>
            {
                ...
                if (!showHidden)
                    ApplyTopicConstraints(storeId, customerRolesIds, out query);
